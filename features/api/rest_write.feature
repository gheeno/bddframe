...
Feature: REST Write — POST, PUT, PATCH, DELETE with request bodies
  ...
  #
  # Run:  bddframe run features/api/rest_write.feature --no-capture
  ...
  Scenario: POST — create a new object
    ...
    When performs a POST call at '/objects' with body '{"name":"BDDFrame Test Device","data":{"year":2026}}'
    Then the response status should be 200
    And the response body should contain 'BDDFrame Test Device'
    And the response body should contain 'id'

  @put
  Scenario: PUT — full replacement of object id 7
    Given sets a request header 'Content-Type' to 'application/json'
    When performs a PUT call at '/objects/7' with body '{"name":"BDDFrame Updated","data":{"year":2026}}'
    Then the response status should be 200
  ...
  Scenario: PATCH — partial update of object id 7
    Given sets a request header 'Content-Type' to 'application/json'
    When performs a PATCH call at '/objects/7' with body '{"name":"BDDFrame Patched"}'
    Then the response status should be 200
