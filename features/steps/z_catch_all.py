...
behave auto-imports every ``features/steps/*.py`` at startup to discover step
definitions. BDDFrame registers exactly ONE step matcher — a regex catch-all
that routes each Gherkin line to the web or visual agent (see
``bddframe.steps.catch_all``). This module just re-exports it so behave picks it
...
"""
from bddframe.steps.catch_all import *  # noqa: F401,F403,E402
