...
Feature: Pure LLM Mode — every step interpreted by the language model
  ...
  Scenario: Full LLM mode — entire login flow interpreted by the model
    ...
    Given the BusterBlock video store is open at "[BUSTERBLOCK]"
    When I sign in as [BB_USER] using the password [BB_PASS]
    Then I should be looking at the VHS movie catalog

  @no_retry
  Scenario: Full LLM mode — catalog interaction in natural language
    Given the user is logged into BusterBlock at "[BUSTERBLOCK]" as [BB_USER] with password [BB_PASS]
    When the catalog is visible
    And I add the first available movie to my cart
