...
Feature: Assertions — every assertion pattern BDDFrame supports
  ...
  Scenario: Assert element is enabled / disabled
    ...
    When User navigates to "[BUSTERBLOCK]/cart.html"
  ...
  Scenario: Store element text, then assert the stored value
    # Store → `VAR` — then assert the variable against a literal.
    When User stores the "VHS Catalog" heading as `HEADING`
  ...
  Scenario: Variable comparison operators
    ...
    Given User sets `PRICE` to '3.99'
    Then `PRICE` should equal '3.99'
    And `PRICE` should not equal '0.00'
    And `PRICE` should be greater than '1.00'
    And `PRICE` should be less than '10.00'
