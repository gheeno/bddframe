...
Feature: Custom Steps — steps not in the built-in dictionary
  ...
  #     Add a @given / @when / @then function to features/steps/custom_hooks.py.
  #     Behave discovers all *.py files in features/steps/ automatically.
  ...
  #     → reads resources/users.csv and logs in as the first row
  #
  ...
  Scenario: Custom step — log in as the first user from a CSV file
    ...
    # It reads resources/users.csv (relative to THIS feature file) and executes
    # the fill + click steps for the first row. See custom_hooks.py for the code.
    Given User is on "[BUSTERBLOCK]"
    When a user from this list "users.csv" logs in
