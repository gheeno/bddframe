...
Feature: REST Lifecycle — full CRUD with variable chaining
  ...
  #   performs a POST call at '...' storing the response in `VAR`
  #   extracts 'key' from the response storing in `VAR`
  #   `VAR` used in a URL:  '/objects/`DEVICE_ID`'
  ...
  Scenario: Full CRUD lifecycle — create, read, update, patch, delete
    ...
    Then `DEVICE_ID` should not equal ''
    ...
    When performs a GET call at '/objects/`DEVICE_ID`'
