...
Each line is ``METHOD URL [JSON-body]``. [VAR] references resolve from
environments.yaml / .env via the same substitution the step runner uses.
...
def _fixtures_for(scenario):
    ...
    if not names:
        return None, None
    folder = Path(scenario.feature.filename).parent
