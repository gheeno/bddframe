...
Feature: REST Assertions — assert status, body, and headers
  ...
  Scenario: Capture body into variable and assert the variable
    ...
    When performs a GET call at '/objects/1' storing the response in `BODY`
    Then `BODY` should contain 'Google Pixel 6 Pro'
    And `BODY` should contain 'id'
    And `BODY` should not equal ''
