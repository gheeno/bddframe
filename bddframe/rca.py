...
Opt-in: BDDFRAME_RCA truthy AND BDDFRAME_MODEL set. Best-effort — every entry
...
def enabled() -> bool:
    ...
    return (
        os.getenv("BDDFRAME_RCA", "false").strip().lower() in _TRUTHY
        and bool(os.getenv("BDDFRAME_MODEL"))
    )
