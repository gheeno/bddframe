...
Feature: REST Resource Payload — load request bodies from files
  ...
  # reference them by relative path. The file content is stored in `PAYLOAD`
  # (and `PAYLOAD_<STEM>` for the named-variable form) and can be used in
  # any subsequent REST call step.
  #
  # Patterns demonstrated:
  #   uses this payload 'payloads/file.json'         — single file → `PAYLOAD`
  ...
  Scenario: POST with a payload loaded from a JSON file
    ...
    # Content is stored in `PAYLOAD`. The POST body references `PAYLOAD`.
  ...
  Scenario: POST then PUT using two payload files chained by extracted ID
    ...
    Then the response status should be 200
    When extracts 'id' from the response storing in `DEV_ID`
