...
from noodle.agent import generate, prompts, validate
...
def test_generation_prompt_embeds_vocabulary_and_request():
    ...
    assert 'User clicks the login button' in p        # vocabulary present
    assert "performs a GET request" in p              # REST family present
...
def test_generate_llm_repairs_unmatched_steps_once(tmp_path, monkeypatch, capsys):
    responses = [UNMATCHED_FEATURE, GOOD_FEATURE]
    calls = []
    ...
    assert "interpretive dance" not in feat.read_text()
...
def test_web_step_without_browser_fails_clearly():
    with pytest.raises(AssertionError, match="@api scenario has no browser"):
...
def test_llm_resolution_cache(monkeypatch):
    ...
    a2 = step_resolver._llm_resolve("User does the thing")
