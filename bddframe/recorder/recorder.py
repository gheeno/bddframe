...
from bddframe.recorder import sensitives

# Injected into every page to capture click and fill events without CSS selectors.
_EVENT_SCRIPT = """
(function() {
  if (window.__bddframe_attached) return;
  window.__bddframe_attached = true;
  ...
  document.addEventListener('click', function(e) {
    ...
    if (label) window.__bddframe_event('click', { label: label });
  }, true);
  ...
  document.addEventListener('change', function(e) {
    ...
    if (tag === 'INPUT' || tag === 'TEXTAREA' || tag === 'SELECT') {
      window.__bddframe_event('fill', { field: getLabel(el), value: el.value });
    }
  }, true);
})();
"""
...
class Recorder:
    ...
    def record(self):
        ...
        with sync_playwright() as pw:
            ...
            print(
                "\n  [BDDFrame Recorder] Browser open — perform your test flow, "
    ...
    def _attach_listeners(self, page):
        page.expose_function("__bddframe_event", self._on_event)
    ...
    def _write_feature(self):
        ...
        print(f"\n  [BDDFrame Recorder] Feature written to: {self.output_path}\n")
