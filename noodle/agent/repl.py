"""noodle-agent — an interactive shell that maps plain English to engine
...
def dispatch(line: str, cfg: dict, workspace: str, llm: str | None) -> bool:
    ...
    if low in {"list", "what tests", "what tests do we have", "list all scenarios"}:
        _noodle("list", cfg["features_dir"], workspace=workspace)
        return True
    ...
    if m:
        from noodle.agent import generate
    ...
    if low.startswith("run"):
        ...
        if not rest or rest.lower() in {"all", "all tests", "the tests", "everything"}:
            _noodle("run", cfg["features_dir"], workspace=workspace)
            return True
        ...
        if feat:
            ...
            _noodle("run", cfg["features_dir"], "--tag", target, workspace=workspace)
        return True
...
def main(argv=None):
    ...
    if llm:
        os.environ["NOODLE_MODEL"] = model or _LLM_MODELS.get(llm, llm)
        if llm == "ollama":
            os.environ.setdefault("NOODLE_LLM_URL", "http://localhost:11434")

    cfg = config.load(workspace)
    print(f"noodle-agent — workspace: {Path(workspace).resolve()}"
